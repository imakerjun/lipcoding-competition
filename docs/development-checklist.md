...
### 기본 설정
- [ ] 프로젝트 디렉토리 생성
- [ ] 프론트엔드 앱 설정 (http://localhost:3000)
- [ ] 백엔드 앱 설정 (http://localhost:8080)
- [ ] 데이터베이스 설정 및 초기화
- [ ] OpenAPI 문서 생성 (openapi.yaml)

## 🔐 인증 시스템

### 회원가입 기능
- [ ] `/signup` 엔드포인트 구현
- [ ] 이메일, 비밀번호, 이름, 역할(mentor/mentee) 입력 받기
- [ ] 회원가입 후 `/` 페이지로 리다이렉트
- [ ] UI: 이메일 필드 (`id="email"`)
...
### 로그인 기능
- [ ] `/login` 엔드포인트 구현
- [ ] JWT 토큰 발급 (RFC 7519 표준 준수)
- [ ] JWT 클레임 포함: `iss`, `sub`, `aud`, `exp`, `nbf`, `iat`, `jti`, `name`, `email`, `role`
- [ ] 토큰 유효기간: 1시간
...
### 프로필 조회
- [ ] `/me` 엔드포인트 구현
- [ ] 멘토/멘티 역할별 다른 프로필 정보 반환
- [ ] 기본 프로필 이미지 설정
  - 멘토: `https://placehold.co/500x500.jpg?text=MENTOR`
  - 멘티: `https://placehold.co/500x500.jpg?text=MENTEE`

### 프로필 수정
- [ ] `/profile` PUT 엔드포인트 구현
- [ ] 멘토: 이름, 소개글, 이미지, 기술 스택 수정
- [ ] 멘티: 이름, 소개글, 이미지 수정
- [ ] 이미지 업로드 기능 (JPG/PNG, 500x500~1000x1000px, 최대 1MB)
- [ ] UI: 이름 필드 (`id="name"`)
- [ ] UI: 소개글 필드 (`id="bio"`)
...
### Swagger UI
- [ ] `/openapi.json` 엔드포인트 제공
- [ ] `/swagger-ui` 페이지 제공
- [ ] 루트 URL에서 Swagger UI로 자동 리다이렉트
...
### 실행 명령어
- [ ] 백엔드 실행 명령어 문서화 (예: `npm start`, `fastapi run`, `./mvnw spring-boot:run`)
- [ ] 프론트엔드 실행 명령어 문서화
- [ ] 데이터베이스 초기화 명령어 문서화
...
- [ ] 백엔드: http://localhost:8080 접속 가능
- [ ] API: http://localhost:8080/api 접속 가능
- [ ] Swagger UI: http://localhost:8080 접속 시 자동 리다이렉트
